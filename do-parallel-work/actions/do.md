...
> **Part of the do-parallel-work skill.** Invoked when routing determines the user is adding a request. Creates a `do-work/` folder in your project for request tracking.
...
**Parallel execution fields (do-parallel-work specific):**

| Field | Required | Description |
|-------|----------|-------------|
| `depends_on` | No | Array of REQ IDs that must complete before this one starts |
| `priority` | No | Execution priority: `high`, `normal` (default), or `low` |
| `source_doc` | No | Path to master document this REQ was derived from |
...
| `source_doc` | No | Path to master document this UR was derived from |

The work action adds additional fields (`claimed_at`, `route`, `completed_at`, `assigned_agent`) as it processes each request.

**Note:** Legacy REQ files may have `context_ref` instead of `user_request`. The work action handles both.

### Dependency Examples

When creating REQs with dependencies:
...
created_at: 2025-01-26T10:00:00Z
user_request: UR-001
depends_on: [REQ-001, REQ-002]  # Must wait for auth and database
priority: normal
---
```

When derived from a master plan document:

```yaml
---
id: UR-005
title: YouTube Accelerator Features
source_doc: /home/user/gdrive-tasks/plans/quant-alchemy-master-plan.md
created_at: 2025-01-26T10:00:00Z
requests: [REQ-020, REQ-021, REQ-022]
---
```
